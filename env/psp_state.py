...
class PSPState:
    ...
    def to_features_and_edge_index(self, normalize):
        ...
        if self.observe_conflicts_as_cliques:
            ...
            rca = np.concatenate(
                [self.resource_conf_id, self.resource_conf_val], axis=-1
            )
        else:
            rce = None
            rca = None
    ...
    def compute_dates_on_affectation(self, node_id):
        ...
        affected_parents = np.where(self.affected(job_parents))[0]
        ...
        self.insert_timepoints_in_resources(
            0, node_id, start[0], self.tct_real(node_id)
        )
        for i in range(3):
            self.insert_timepoints_in_resources(
                i + 1, node_id, start[i + 1], self.tct(node_id)[i]
            )
    ...
    def add_resource_precedence(self, prec, succ, on_start, critical, timetype):
        ...
        self.resource_prec_att.append((on_start, critical, timetype))
    ...
    def insert_timepoints_in_resources(self, timeindex, node_id, start, end):
        ...
        for r in resources_used:
            self.resource_timelines[r][timeindex].consume(
                node_id, self.resource_usage(node_id, r), start, end
            )
